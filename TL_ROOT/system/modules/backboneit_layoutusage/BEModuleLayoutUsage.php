...
class BEModuleLayoutUsage extends BackendModule {
	...
	public function setLayout($varLayout) {
		if(is_object($varLayout)) {
			...
			Database::getInstance()->fieldExists('fallback', 'tl_layout') && $fallback = ', fallback';
			$sql = 'SELECT id, name' . $fallback . ' FROM tl_layout WHERE id = ?';
			$this->arrLayout = Database::getInstance()->prepare($sql)->execute($varLayout)->row();
		}
	}
